...
- public `AiVerseDataWorkspaceManager` under `@ai-verse/data/native`;
- read-only one-workspace resolution from trusted AI-Verse OS root + workspace ID;
- exact native workspace slug contract `^[a-z0-9][a-z0-9-]*# AI-Verse Data Build Map
...
;
- bounded safe `WORKSPACE.yaml` loading;
- required workspace `schema_version`, `id`, `name`, `type`, `status`, and `purpose` validation;
- supported workspace schema major 2;
- unknown/additive manifest metadata tolerance;
- exact requested/directory/manifest workspace identity match;
- active / paused / archived status distinction;
- canonical path derived only as `workspaces/<id>/data/ai-verse-data.sqlite`;
- safe data-directory/database symlink and wrong-type rejection;
- exact existing database discovery without whole-workspace-tree guessing;
- distinct clean missing, orphan residue, and legacy unbound states;
- existing recovery states preserved, including migration-required/incomplete, quarantine, unsupported, scope conflict, and unavailable;
- no silent adoption of healthy but unbound legacy databases;
- clean-missing initialization requires current enabled Task 20 installation;
- fresh initialization allowed only for active workspaces;
- same-directory staged SQLite bootstrap;
- staged database receives exact workspace binding before publication;
- staged integrity/reopen verification before publication;
- atomic no-overwrite hard-link publication;
- concurrent initializers converge on one canonical DB without replacement;
- repeated exact-binding initialization returns `already_initialized`;
- one requested workspace only, no mass initialization;
- Task 20 registry/owned files preserved.
...
Do not begin Task 23 / 41 until Task 22 is implemented, verified, committed, logged in the continuation handoff, and reported complete.
